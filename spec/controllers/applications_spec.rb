RSpec.describe Controllers::Applications do
  ...
  describe 'GET /' do
    ...
    describe 'Nominal case' do
      before do
        get "/#{application.id.to_s}?redirect_uri=#{application.redirect_uris.first}"
      end
    ...
    describe 'Alternative case with parameters in the URI' do
      ...
      before do
        get "/#{application.id.to_s}?redirect_uri=#{CGI.escape(uri)}"
      end
      it 'Returns a 200 (OK) status code' do
        expect(last_response.status).to be 200
      end
    ...
    describe 'when the redirection URI is not in the application' do
      before do
        get "/#{application.id.to_s}?redirect_uri=unknown"
      end
      it 'Returns a 404 (Not Found) status code' do
        expect(last_response.status).to be 404
      end
